#!/bin/bash
#
# clean-inventory.sh is Subset of th auto-vaquero.sh 
# Validate and generate inventory file in CWD.
# version = 7.1
...
# 7.0 clean-inventory to clean existing inventory file in cwd based on input csv file.
# 7.1 efects for duplicate check to be more precise.. 
#
#######################################################################
csvfile=$1
...
#environment data,possibly pulled from env.yml
...
[ ! -f $inventory ] && { echo "$inventory file not found"; exit 99;}
#Disabling this for the generate inventory since it is not required.
...
do
  ...
  if [ ! -z "$ipmi" ];then
     ...
     echo "${blue}$host, removing inventory configuration${normal}"
     clear_config $host 
  #cos3260
  elif [ ! -z "$cimc" ];then
     echo "${blue}$host, removing inventory configuration${normal}"
